...
void ParallelCleaningTask::work(uint worker_id) {
  // printf("[complete_cleaning %d] _code_cache_task\n", worker_id);
  {
  ...
  // printf("[complete_cleaning %d] _string_symbol_task\n", worker_id);

  {
    // Clean the Strings and Symbols.
    _string_symbol_task.work(worker_id);
  }
  // printf("[complete_cleaning %d] _resolved_method_cleaning_task\n", worker_id);

  {
    // Clean unreferenced things in the ResolvedMethodTable
    _resolved_method_cleaning_task.work();
  }
  // printf("[complete_cleaning %d] _code_cache_task2\n", worker_id);
  ...
  if (_unloading_occurred) {
    // printf("[complete_cleaning %d] _klass_cleaning_task\n", worker_id);
    // Clean all klasses that were not unloaded.
    _klass_cleaning_task.work();
  }
  // printf("[complete_cleaning %d] finish\n", worker_id);
}
